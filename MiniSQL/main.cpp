...
void run()
{
	...
	case NUMBERWRONG:
		...
		break;
	case EXECFILE:
		fstream fin(analysis.filename.c_str(), ios::in);
		strcpy(cmd, "");
		while (fin.getline(cmd, 500))
		{
...
void visual(display afterselect, table temptable, vector<attribute> mulobject)
{
	if (mulobject[0].name == "*")
	{
		...
		{
			for (int j = 0;j < temptable.attributes[i].length;j++)
				cout << "-";
			cout << "+";
		}
		cout << endl;
		...
		for (int j = 0;j < temptable.amount;j++)
		{
			for (int k = 0;k < temptable.attributes[j].length;k++)
				cout << "-";
			cout << "+";
		}
		cout << endl;
	...
	else
	{
		...
		for (int i = 0;i < mulobject.size();i++)
		{
			...
			for (selected = 0;selected < temptable.attributes.size();selected++)
				if (temptable.attributes[selected].name == mulobject[i].name)
					break;
			for (int j = 0;j < temptable.attributes[selected].length;j++)
				cout << "-";
			cout << "+";
		}
		cout << endl;
		...
		for (int i = 0;i < afterselect.displays.size();i++)
		{
			...
			for (int j = 0;j < mulobject.size();j++)
			{
				int selected;
				for (selected = 0;selected < temptable.attributes.size();selected++)
					if (temptable.attributes[selected].name == mulobject[j].name)
						break;
